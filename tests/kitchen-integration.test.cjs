...
test("merchant receives private version history and admin review notes without exposing them in public summaries", async () => {
  const designId = "12345678-1234-4234-8234-123456789abc";
  const v1 = "22345678-1234-4234-8234-123456789abc";
  const v2 = "32345678-1234-4234-8234-123456789abc";
  const rows = {
    merchant_stores: [
      {
        id: "factory-1",
    ...
    kitchen_design_versions: [
      {
        ...
        exclusions: [],
        cabinet_count: 2,
        min_room_width_mm: 4000,
        min_room_depth_mm: 3000,
        max_height_mm: 2600,
        created_at: "2026-09-23T03:00:00Z",
        submitted_at: "2026-09-23T03:10:00Z",
        approved_at: null,
      },
      {
        id: v1,
        ...
        submitted_at: "2026-09-22T03:10:00Z",
        approved_at: "2026-09-22T04:00:00Z",
      },
    ],
    kitchen_design_media: [
      {
        ...
        kind: "thumbnail",
        source: "system",
        url: "https://res.cloudinary.com/demo/v2.webp",
        alt_text: "Oak",
        is_primary: true,
        sort_order: 0,
      },
    ...
    kitchen_design_reviews: [
      {
        ...
        note: "Бариулын сонголтыг тодруулна уу.",
        created_at: "2026-09-23T03:20:00Z",
      },
    ],
  };
  const db = {
    from(table) {
      ...
      const chain = {
        ...
        eq(column, value) {
          data = data.filter(
            (item) => !(column in item) || item[column] === value,
          );
          return chain;
        },
        in(column, values) {
  ...
  const server = loadSource("src/lib/kitchenMarketplaceServer.ts", {
    "@/lib/supabase/admin": { getSupabaseAdmin: () => db },
  });
  const result = await server.readMerchantKitchenDesigns("merchant", db);
  assert.equal(result.designs[0].versions.length, 2);
  assert.equal(result.designs[0].versions[1].isPublished, true);
  assert.equal(
    result.designs[0].reviews[0].note,
    "Бариулын сонголтыг тодруулна уу.",
  );
  const publicResult = await server.readPublishedKitchenDesigns(db);
  assert.equal(publicResult[0].versionId, v1);
  assert.deepEqual(publicResult[0].versions, []);
  assert.deepEqual(publicResult[0].reviews, []);
});

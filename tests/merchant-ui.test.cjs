...
function harness(role = "merchant") {
  let stateValues = [], index = 0;
  const changes = [], requests = [];
  const auth = { user: { id: "owner", name: "Мөнх", email: "owner@example.com" }, role, initialized: true, initialize: async () => {} };
  const useAuth = Object.assign(selector => selector(auth), { getState: () => auth });
  const mocks = {
    react: { ...React, useEffect() {}, useState(initial) { const i = index++; return [i in stateValues ? stateValues[i] : typeof initial === "function" ? initial() : initial, next => changes.push({ index: i, next })]; } },
    "next/navigation": { useRouter: () => ({ replace() {} }) },
    "@/store/auth": { useAuth },
    "@/lib/authFetch": { authFetch: async (path, init, owner) => { requests.push({ path, init, owner }); return { ok: true, json: async () => ({ ok: true, id: "product-id" }) }; } },
    "@/store/catalog": { useCatalogStore: { getState: () => ({ refresh() {} }) } },
  };
  return { auth, mocks, requests, changes, reset(values = []) { stateValues = values; index = 0; } };
}
...
const order = (status, paymentStatus = "paid") => ({
  id: "12345678-1234-1234-1234-123456789abc", status, paymentStatus, currency: "MNT", subtotal: 200000, created_at: "2026-09-15T12:00:00Z",
  delivery: { name: "Хүлээн авагч", phone: "99000000", address: "Улаанбаатар" },
  items: [{ productId: "owned-product", name: "Модон сандал", color: "oak", colorName: "Царс", material: "wood", materialName: "Мод", qty: 2, unitPrice: 100000, lineTotal: 200000 }],
});

function renderOrder(h, value, onUpdated) {
  const { MerchantOrders } = loadSource("src/components/MerchantOrders.tsx", h.mocks);
  h.reset([0, 0, { orders: [value], hasMore: false }, false, null]);
  const list = MerchantOrders({ owner: "owner" });
  const card = find(list, node => typeof node.type === "function" && node.type.name === "MerchantOrderCard");
  assert.ok(card);
  h.reset();
...
test("merchant order actions advance only the next paid fulfillment step", async () => {
  for (const [current, next] of [["pending", "processing"], ["processing", "shipped"], ["shipped", "delivered"]]) {
    const h = harness();
    ...
    assert.deepEqual(JSON.parse(h.requests[0].init.body), { id: order(current).id, status: next, expectedStatus: current });
  }
});

test("unpaid, cancelled and completed merchant orders have no fulfillment or payment action", () => {
  for (const value of [order("pending", "pending_payment"), order("processing", "cancelled"), order("delivered")]) {
    const h = harness();
...
test("a delayed order response cannot update UI after switching merchant accounts", async () => {
  const h = harness();
  let release;
  h.mocks["@/lib/authFetch"].authFetch = () => new Promise(resolve => { release = resolve; });
  const updates = [];
  const tree = renderOrder(h, order("pending"), status => updates.push(status));
  ...
  assert.ok(!h.changes.some(change => typeof change.next === "string" && change.next.includes("шинэчлэгдлээ")));
});

test("a rejected order transition keeps current status and displays the server conflict", async () => {
  const h = harness();
  const conflict = "Захиалгын төлөв өөрчлөгдсөн байна. Жагсаалтаа шинэчилнэ үү.";
  h.mocks["@/lib/authFetch"].authFetch = async () => ({ ok: false, status: 409, json: async () => ({ error: conflict }) });
  const updates = [];
  const tree = renderOrder(h, order("pending"), status => updates.push(status));
...
test("admin role editor never exposes admin promotion and saves merchant only after explicit action", async () => {
  ...
  const user = { id: "target", name: "Бат", email: "bat@example.com", orders: 0, joined: "2026-01-01", role: "customer", store: null };
  h.reset(["", 1, 0, { page: 1, users: [user], hasMore: false }, false, null]);
  const tree = list.type(list.props);
  const editor = find(tree, node => typeof node.type === "function" && node.type.name === "UserRoleEditor");
  h.reset();
  const initial = editor.type(editor.props);
  assert.equal(find(initial, node => node.type === "button"), undefined);
  ...
  assert.deepEqual(JSON.parse(h.requests[0].init.body), { id: "target", role: "merchant" });
  assert.equal(h.requests[0].path, "/api/admin/users");
  h.reset();
  const adminRow = editor.type({ ...editor.props, user: { ...user, role: "admin" } });
  assert.equal(find(adminRow, node => node.type === "select"), undefined);
});
...
test("editing merchant product preserves all variants and stock version", async () => {
  ...
  const list = find(workspaceTree, node => typeof node.type === "function" && node.type.name === "MerchantProducts");
  const product = {
    id: "p-1", name: "Сандал", category: "office", description: "Тайлбар", image: "/product.jpg", images: ["/extra.jpg"], basePrice: 2000, rating: 0, reviewCount: 0,
    colors: [{ id: "oak", name: "Царс", hex: "#cccccc", priceDelta: 200 }, { id: "black", name: "Хар", hex: "#000000", priceDelta: 300 }],
    materials: [{ id: "wood", name: "Мод", priceDelta: 0 }, { id: "metal", name: "Металл", priceDelta: 100 }], defaultColor: "black",
    stockQuantity: 3, inStock: true, dimensions: { w: 0.6, d: 0.5, h: 0.8 },
  };

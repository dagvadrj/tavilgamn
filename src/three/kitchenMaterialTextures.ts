...
import { KITCHEN_TEXTURE_KINDS, type KitchenTextureKind } from "@/lib/kitchenMaterials";

export type KitchenMaterialTextureSet = Partial<Record<KitchenTextureKind, THREE.Texture>>;
...
function acquireTexture(kind: KitchenTextureKind, url: string) {
  ...
  if (!entry) {
    ...
    entry = { refs: 0, promise: Promise.resolve(null as unknown as THREE.Texture) };
    const pending = entry;
    pool.set(key, entry);
  ...
  return {
    ...
    release() {
      ...
      void owned.promise.then(() => {
        ...
        if (typeof owned.idle === "object" && "unref" in owned.idle) owned.idle.unref();
      }, () => undefined);
    },
  };
}

export function acquireKitchenMaterialTextures(paths: Record<string, string> | undefined) {
  ...
  let released = false;
  return {
    promise: Promise.allSettled(leases.map(async (lease) => [lease.kind, await lease.promise] as const))
      .then((results) => Object.fromEntries(results.flatMap((result) => result.status === "fulfilled" ? [result.value] : [])) as KitchenMaterialTextureSet),
    release() {
      if (released) return;
      released = true;

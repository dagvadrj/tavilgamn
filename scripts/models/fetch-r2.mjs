...
if (
  ...
  throw new Error("Use --output folder --limit positive-integer");
...
try {
  ...
  for (let offset = 0; ; offset += 100) {
    const { data, error } = await db
      ...
      .select("id,name,glb_path")
      .order("id")
      .range(offset, offset + 99);
    if (error)
      throw new Error(`Could not read model catalogue (${error.code})`);
    models.push(...data);
  ...
  for (const model of models) {
    ...
    if (head.ContentLength > 200 * 1024 * 1024) {
      console.log(`Skip oversized source: ${model.name}`);
      continue;
    }
    sources.push({
      id: model.id,
  ...
  for (const source of sources.slice(0, Number(values.limit))) {
    ...
    console.log(
      `${source.name}: ${(source.bytes / 1048576).toFixed(2)} MiB -> ${file}`,
    );
  }
  await writeFile(
    path.join(values.output, "sources.json"),
    JSON.stringify(downloaded, null, 2),
    { flag: "wx" },
  );
  console.log(
    `Downloaded ${downloaded.length} source(s). Remote files unchanged.`,
  );
} finally {
  client.destroy();
}
